...
func (info *faucetInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Baquaseum listener port":       strconv.Itoa(info.node.portFull),
		"Funding amount (base tier)":   fmt.Sprintf("%d Baquass", info.amount),
...
func checkFaucet(client *sshClient, network string) (*faucetInfos, error) {
	...
	return &faucetInfos{
		node: &nodeInfos{
			...
			portFull: infos.portmap[infos.envvars["BAQ_PORT"]+"/tcp"],
			ethstats: infos.envvars["BAQ_NAME"],
			keyJSON:  keyJSON,
			keyPass:  keyPass,
		},
		host:          host,
