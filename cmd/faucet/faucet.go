...
// faucet is a Baquas faucet backed by a light client.
package main
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Baquaseum protocol")
	statsFlag   = flag.String("ethstats", "", "Ethstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Baquass to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Baquaseum and the faucet")
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Baquass", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Baquaseum address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Baquaseum light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Baquaseum protocol stack
	client *ethclient.Client   // Client connection to the Baquaseum chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Baquaseum light client protocol
...
// close terminates the Baquaseum connection and tears down the faucet.
...
// apiHandler handles requests for Baquas grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the Baquaseum address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
...
// the username, avatar URL and Baquaseum address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for Baquaseum addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Baquaseum address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Baquaseum address and profile URL.
...
// returning the username, avatar URL and Baquaseum address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Baquaseum address,
