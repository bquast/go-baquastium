...
const (
	...
	URN_WANBaquasnetLinkConfig_1    = "urn:schemas-upnp-org:service:WANBaquasnetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Baquasnet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Baquasnet
//
...
// WANBaquasnetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANBaquasnetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANBaquasnetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANBaquasnetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANBaquasnetLinkConfig1Clients() (clients []*WANBaquasnetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANBaquasnetLinkConfig_1); err != nil {
		return
	}
	clients = newWANBaquasnetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANBaquasnetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANBaquasnetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANBaquasnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANBaquasnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANBaquasnetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANBaquasnetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANBaquasnetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANBaquasnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANBaquasnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANBaquasnetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANBaquasnetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANBaquasnetLinkConfig1 {
	clients := make([]*WANBaquasnetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANBaquasnetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewBaquasnetLinkStatus: allowed values: Up, Down
func (client *WANBaquasnetLinkConfig1) GetBaquasnetLinkStatus() (NewBaquasnetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewBaquasnetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANBaquasnetLinkConfig_1, "GetBaquasnetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewBaquasnetLinkStatus, err = soap.UnmarshalString(response.NewBaquasnetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
