...
enum ethash_io_rc {
	BAQASH_IO_FAIL = 0,           ///< There has been an IO failure
	BAQASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	BAQASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	BAQASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
 ...
 * ethash_full_t
 */
#ifdef BAQASH_PRINT_CRITICAL_OUTPUT
#define BAQASH_CRITICAL(...)							\
	do													\
	{													\
		printf("BAQASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
